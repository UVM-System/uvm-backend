...
/**
查询时间区间内指定交易状态的OrderList
*/
func (o *Order) GetOrderListWithinSpan(businessId uint, status bool, startTime time.Time, endTime time.Time) (orders []Order, err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("model.GetOrderListWithinSpan: %w", err)
		}
	}()
